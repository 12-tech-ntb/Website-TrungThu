...
// --- TREO ẢNH LÊN CÂY ---
const hangingPhotosGroup = new THREE.Group();
treeGroup.add(hangingPhotosGroup);
...
const photoCount = 6; // Bạn có thể tăng số lượng ảnh muốn treo
const textureLoader = new THREE.TextureLoader();
const hangingPhotos = []; // Mảng lưu trữ để làm hiệu ứng gió thổi

for (let i = 0; i < photoCount; i++) {
  // Lấy ngẫu nhiên ảnh trong thư mục
  const tex = textureLoader.load(
    photoUrls[Math.floor(Math.random() * photoUrls.length)],
  );
  tex.colorSpace = THREE.SRGBColorSpace; // Giúp ảnh không bị nhạt màu

  // Tạo khung ảnh Polaroid (tỉ lệ 3:4)
  ...
  // Tạo viền trắng đằng sau (cho giống ảnh in)
  const frameGeo = new THREE.PlaneGeometry(1.35, 1.9);
  ...
  // Xếp ảnh đè lên viền trắng
  photoMesh.position.set(0, 0.05, 0.01);
  frameMesh.position.set(0, 0, 0);

  // Nhóm ảnh và viền lại, đặt tên là "pivot" (tâm xoay)
  const pivot = new THREE.Group();
  const cardGroup = new THREE.Group();
  cardGroup.add(frameMesh);
  cardGroup.add(photoMesh);

  // Kéo ảnh xuống dưới tâm xoay để khi xoay giống như đang bị treo
  cardGroup.position.set(0, -0.9, 0);
  pivot.add(cardGroup);

  // Random vị trí treo trên tán cây
  // 1. Phân bổ đều 360 độ (chia hình tròn thành các phần bằng nhau thay vì random hoàn toàn)
  const angle = (i / photoCount) * Math.PI * 2 + Math.random() * 0.5;

  // 2. Tăng bán kính (radius) lên mức 6.5 - 8.0 để ảnh nhô ra hẳn bên ngoài lớp lá (Lớp lá đang có bán kính khoảng 6-7)
  const radius = 6.5 + Math.random() * 1.5;
  const yPos = 4.5 + Math.random() * 3.5;

  pivot.position.set(Math.cos(angle) * radius, yPos, Math.sin(angle) * radius);

  // 3. Xoay mặt ảnh hướng ra ngoài để người dùng luôn nhìn thấy mặt trước của ảnh
  pivot.rotation.y = -angle + Math.PI / 2 + (Math.random() - 0.5) * 0.4;

  hangingPhotosGroup.add(pivot);

  // Lưu thuộc tính để xử lý animation đung đưa
  hangingPhotos.push({
    mesh: pivot,
...
function createRabbit() {
  ...
  // Vật liệu chính (lông trắng tinh, tăng độ nhám để trông giống lông bông)
  const rabbitMat = new THREE.MeshStandardMaterial({
    color: 0xffffff,
    roughness: 0.9,
  });

  // Vật liệu phụ (hồng cho tai/mũi, đen cho mắt)
  ...
  // 4. Tai (dùng hình cầu ép dẹt thay vì hình trụ cứng ngắc như cũ)
  const earGeo = new THREE.SphereGeometry(0.08, 12, 12);
  earGeo.scale(1, 3.5, 0.4);

  // --- Tai trái ---
  const earLeft = new THREE.Mesh(earGeo, rabbitMat);
  earLeft.position.set(-0.15, 0.95, 0.25);
  earLeft.rotation.set(-0.2, 0, 0.3); // Ngả ra sau và dạt sang bên
  group.add(earLeft);

  // Lõi tai trái (màu hồng)
  const innerEarLeft = new THREE.Mesh(earGeo, pinkMat);
  innerEarLeft.scale.set(0.6, 0.8, 1.1);
  innerEarLeft.position.set(0, 0, 0.02); // Nổi nhẹ lên mặt trước
  earLeft.add(innerEarLeft);

  // --- Tai phải (nhân bản từ tai trái) ---
  const earRight = new THREE.Mesh(earGeo, rabbitMat);
  earRight.position.set(0.15, 0.95, 0.25);
  earRight.rotation.set(-0.2, 0, -0.3);
  group.add(earRight);

  const innerEarRight = innerEarLeft.clone();
  earRight.add(innerEarRight);

  // 5. Mắt (2 chấm đen nhánh)
  const eyeGeo = new THREE.SphereGeometry(0.035, 8, 8);
  const eyeL = new THREE.Mesh(eyeGeo, blackMat);
  eyeL.position.set(-0.12, 0.72, 0.6);
  group.add(eyeL);
...
const wishList = [
  ...
  {
    text: "Yêu em 😘, chúc em luôn hạnh phúc và bình an trong cuộc sống.",
    img: "./assets/3.jpg",
  },
  {
...
// === CHÈN ĐOẠN CODE NÀY VÀO ĐÂY ===
// --- XỬ LÝ MÀN HÌNH HƯỚNG DẪN ---
const guideOverlay = document.getElementById("guideOverlay");
const startBtn = document.getElementById("startBtn");

startBtn.addEventListener("click", () => {
  // Ẩn màn hình hướng dẫn
  guideOverlay.classList.add("hidden");

  // Tự động phát nhạc khi người dùng tương tác (nhấn nút bắt đầu)
  if (!isPlaying) {
    bgm
      .play()
      .then(() => {
